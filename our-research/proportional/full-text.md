Proportional Mathematics — Patrick's Framework

⚗️ Proportional Mathematics
...
1. Numbers as Fill-States in Vessels
...
For any capacity n, a vessel (0→n)/n exists.
Numbers live inside containers. The container defines the scale.

Any fill-state can be scaled by multiplication.
Multiplication is the primary operation — proportional scaling. Addition is a derivative projection.

Fill-states have direction (±).
Allowing interference patterns — constructive and destructive.

Vessels can be compared and composed.
Establishing ratios between different units. This is where measurement lives.
...
2. The +1 as Construction History
...
3. Irrationals as Resolution Loss

Standard math says √2 is irrational — it can't be expressed as a ratio of integers. Patrick reframes this:

"√ asks what lesser factor exists within a value. 2 and 3 do not contain lesser factors, they are considered prime. One dimensional. √2 or √3 is therefore mathematically viable but lacks a reference frame. Like over-pixelation, the value has gone below its reference and has lost resolution if not accuracy."
— Patrick, on irrationals
...
4. The Dimensional Ceiling

LevelWhat it isExamples
0DThe point which has no partPosition, identity
1DIncremental count or indexNumber line, sequence
2DA flat shape with boundsCircle, triangle, line, square
3DA shape with depth, minimum four verticesTetrahedron, sphere, cube
+DNOT higher dimensions. Faceted objects within 3D.Platonic solids, polyhedra
...
Base: 
Power: 
Offset: -1 (reduction)+1 (growth)

Reading the Events

ExpressionValueReading
1^n − 10Collapse. Any capacity, zeroed. annihilation
1^n + 12Invariance. Any capacity, always 2. birth
2⁵ − 131Reduction from 32. "Made of twos." prime
3³ + 128Growth from 27. "Made of threes." perfect number
4³ − 163Reduction from 64. "Made of fours." = 7×9
5⁴ + 1626Growth from 625. "Made of fives." = 2×313

"These expressions are not claims about factorization, but illustrations of construction history and offset behavior."
— Patrick

🔗 Connection: Euclid Revised

Patrick isn't replacing Euclid — he's addressing what Euclid left open:

Equality: Two ratios can arrive at the same closure by different histories. Equality may be terminal, not procedural.

Limits: Unending numerical expressions don't imply unfinished construction. π is constructed and complete — it just looks infinite in base 10.

Incommensurables: Proportions need not admit integer commensuration, yet still possess closure.

Contradiction: Unnecessary. Real things can't be contradictory — they just exist.

6. The 3×3 Power Spectrum
...
At n=3: 2^4 = 4^2 = 16 — exact commutation! The only integer solution to x^y = y^x (besides x=y).

At n≈3.3: 2.3^3.3 ≈ 3.3^2.3 — the (a,b) pair nearly commutes.

At n=e: no single pair is especially close, but e is where x^(1/x) peaks — the theoretical maximum of "commutability." All pairs that DO commute at other n values owe their existence to this peak.

As n grows larger, ALL pairs diverge further — exponentiation becomes increasingly non-commutative at scale.

🔗 The Ghost of e

Patrick calls this "the ghost of e" — it keeps showing up uninvited in his power tables. It's not put there by hand. It emerges from the structure of exponentiation itself.
...
8. The 8n+1 Recurrence & Sierpiński

Start with seed 1. Apply f(x) = 8x + 1 repeatedly:

Now decompose each term. The recurrence A(n) = 8·A(n−1) + 1 splits into:

B(n) = 8^n (the "skeleton" — pure scaling without memory)

C(n) = (8^n − 1)/7 (the "structure" — accumulated +1 terms)
...
nA(n) = 8n+1 from 1B(n) = 8^nC(n) = holesCarpet depth
0110Solid square
19811 hole
2736499 holes
35855127373 holes
446814096585585 holes
...
— Patrick, responding to the 137 observation
...
9. The Self-Ratio: Where +1 Comes From
...
i=1: 8 x 17 + 128/128 = 137

i=2: 8 x 137 + 1024/1024 = 1097

i=3: 8 x 1097 + 8192/8192 = 8777
...
Rowi=0i=1i=2i=3i=4Sequence
(ab+ab)21713710978777Physics (8n+1)
(ab)18645124096Pure scale
...
"The +1 is not a number — it is what happens when a scale recognizes itself."
— From the wet-math #general discussion, March 2026
...
The pi Row: Seed 1/3, Operation +1
...
The e Row: Seed 4/3, Operation -1
...
ConstantSeedOperationSequenceApproximation
pi1/k = 1/3+1 (deposit)7, 22, 67, 202...22/7 ≈ pi
...
kDimensionFractalRecurrenceNotable value
3d = log2(3) ≈ 1.58Sierpinski triangle3n±122/7 ≈ pi
8d = 3Sierpinski carpet8n+11/137 ≈ alpha
...
Euler's Identity as Proportional Stasis
...
11. The Commutator Connection

🌊 Where Patrick meets the Groovy Commutator
...
The Squaring the Circle (Almost)
...
12. Further Questions from Liet

The +1 history and the commutator: If (abf+a+1)/n ≠ (abf+b+1)/n is the "construction commutator," can we define a formal operator [a,b]_c = (a+1) − (b+1) = a − b and study its properties? Does this commutator have eigenvalues? Fixed points?

The ⅔ rounding threshold: In the McGuffin game system, Patrick uses 0.667 instead of 0.5 as the rounding boundary. Is there a proportional-axiom derivation of why 2/3 is the "right" threshold? It feels logarithmic rather than arithmetic.

Irrationals as fractal boundaries: If √2 is "below resolution," and the Sierpiński carpet is the geometry of the 8n+1 sequence... are irrationals the boundaries between carpet levels? Does "resolution" correspond to fractal depth?

The e-crossover in higher constructions: The power spectrum crossover happens at e for pairs. What happens for triples? For the full tetrahedron? Does the "ghost of e" generalize to higher-arity power structures?

The shell decomposition: (i+1)^(i+1) = i^i + i^(i+1) + (i+1)^i + remainder, and remainder/(i(i+1)) gives clean integers. Why? What's the algebraic identity underlying this? Is this related to the binomial theorem applied to exponentiation?

Connection to E₈: The σ₃ divisor sums connecting the 8n+1 structure sequence to Eisenstein series E₄ (which governs E₈ lattice theta functions) — is this a deep connection or a coincidence of modular arithmetic? The fact that three unrelated areas (Sierpiński holes, octal repunits, modular form coefficients) agree on the same numbers feels significant.